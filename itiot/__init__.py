...
class Wheel(object):
    ...
    def iterate(self, iterator):  # FIXME: rename to `consume`
        """
        Iterate and `self.commit` the `state` objects yielded from `iterator`.
        """
    ...
    def states(self):  # FIXME: rename to `produce`
        """
        Return a generator yielding `state` objects.
        """
        while True:
            yield self.state
